...
export async function onRequestPost({ request, env }) {
  ...
  const { data: account, error: accountError } = await supabase
    ...
    .maybeSingle();
  if (accountError || !account || account.status !== 'available' || (account.moderation_status && account.moderation_status !== 'approved')) {
    return json({ error: 'This listing is no longer available.' }, 409);
  }

  const amount = Math.round(Number(account.price) * 100);
  if (!Number.isSafeInteger(amount) || amount < 100) return json({ error: 'This listing has an invalid price.' }, 400);
  ...
  if (!razorpayResponse.ok || !order.id) {
    console.error('Razorpay order creation failed', { status: razorpayResponse.status, reason: order.error?.description });
