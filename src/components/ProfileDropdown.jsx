...
export default function ProfileDropdown({ user, onLogout }) {
  ...
  useEffect(() => {
    ...
    const loadPublicId = async () => {
      ...
      const [{ data, error }, { data: seller }] = await Promise.all([
        supabase.rpc('get_my_public_id'),
        supabase.from('seller_profiles').select('status').eq('user_id', user.id).maybeSingle(),
      ]);
      if (active && !error) setPublicId(data);
      if (active) setSellerStatus(seller?.status || null);
    };
    loadPublicId();
    return () => { active = false; };
  }, [user?.id]);
  ...
  const itemClass = 'flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left text-sm font-medium text-zinc-800 hover:bg-zinc-50';

  return <div>
    <button onClick={() => setOpen(true)} aria-label="Open profile menu" className="relative grid h-9 w-9 place-items-center overflow-hidden rounded-full bg-yellow-300 text-xs font-black text-[#171206] transition-transform duration-75 hover:scale-[1.04]">{avatarUrl ? <img src={avatarUrl} alt="" className="h-full w-full object-cover" /> : avatarLetter}<span className="absolute bottom-0 right-0 h-2.5 w-2.5 rounded-full border-2 border-white bg-emerald-400" /></button>
    {open && createPortal(<div className="fixed inset-0 z-[9999] flex justify-end">
      ...
      <aside className="relative z-10 flex h-[100dvh] max-h-[100dvh] w-full max-w-[350px] transform-gpu flex-col overflow-hidden border-l border-zinc-200 bg-white p-4 opacity-100 shadow-2xl sm:p-5" aria-label="Buyer account menu">
        <div className="flex shrink-0 justify-end"><button onClick={() => setOpen(false)} className="grid h-9 w-9 place-items-center rounded-full border border-zinc-200 text-zinc-500 hover:bg-zinc-50 hover:text-zinc-900" aria-label="Close profile menu"><svg className="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeWidth="1.8" d="m6 6 12 12M18 6 6 18" /></svg></button></div>
        <div className="mt-3 flex shrink-0 items-center gap-3 rounded-2xl bg-zinc-100 p-3"><span className="grid h-10 w-10 shrink-0 place-items-center overflow-hidden rounded-xl bg-zinc-950 text-sm font-bold text-white">{avatarUrl ? <img src={avatarUrl} alt="Profile" className="h-full w-full object-cover" /> : avatarLetter}</span><div className="min-w-0"><p className="truncate text-base font-bold tracking-[-0.02em] text-zinc-950">{displayName}</p><p className="mt-0.5 text-[10px] font-medium tabular-nums text-zinc-500">ID {publicId || '••••••'}</p></div></div>
        <div className="mt-4 shrink-0 rounded-2xl border border-zinc-200 bg-zinc-50 p-3">
          ...
          <div className="mt-2 grid grid-cols-4 gap-1.5">{MARKETPLACE_THEMES.map((option) => <button key={option.id} type="button" title={option.label} aria-label={`Use ${option.label} theme`} aria-pressed={theme === option.id} onClick={() => setTheme(applyTheme(option.id))} className={`flex min-w-0 flex-col items-center gap-1.5 rounded-xl border px-1 py-2 ${theme === option.id ? 'border-zinc-400 bg-zinc-100' : 'border-transparent hover:bg-zinc-100'}`}><span className="flex -space-x-1">{option.colors.map((color) => <span key={color} className="h-3.5 w-3.5 rounded-full border border-white/50 shadow-sm" style={{ backgroundColor: color }} />)}</span><span className="w-full truncate text-center text-[9px] font-medium text-zinc-600">{option.label.replace('Original ', '')}</span></button>)}</div>
        </div>
        <p className="mt-3 shrink-0 px-1 text-[9px] font-bold uppercase tracking-[0.16em] text-zinc-400">Account</p>
        <div className="mt-2 min-h-0 flex-1 space-y-1 overflow-y-auto overscroll-contain pr-1">
          ...
          <button onClick={() => goTo('/seller-chat')} className={itemClass}><span className="text-zinc-500"><TicketIcon /></span><span className="flex-1">Seller chats</span><span className="text-zinc-400"><ArrowIcon /></span></button>
          ...
          {sellerStatus === 'approved' ? <button onClick={() => goTo('/my-products')} className={itemClass}><span className="text-zinc-500"><StoreIcon /></span><span className="flex-1">My products</span><span className="text-zinc-400"><ArrowIcon /></span></button> : <button onClick={() => goTo('/become-a-seller')} className={itemClass}><span className="text-zinc-500"><StoreIcon /></span><span className="flex-1">Become a seller</span><span className="text-zinc-400"><ArrowIcon /></span></button>}
        </div>
        <div className="shrink-0 border-t border-zinc-200 pt-2 pb-[env(safe-area-inset-bottom)]"><button onClick={() => setConfirmLogout(true)} className="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left text-sm font-semibold text-red-600 hover:bg-red-50"><svg className="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.8" d="M14 8l4 4-4 4M18 12H7m4 8H5a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h6" /></svg>Log out</button></div>
      </aside>
      {confirmLogout && <div className="absolute inset-0 z-20 grid place-items-center bg-zinc-950/45 px-5 backdrop-blur-[2px]" role="dialog" aria-modal="true" aria-labelledby="logout-title">
        <div className="w-full max-w-xs rounded-2xl bg-white p-5 shadow-2xl">
          ...
          <h2 id="logout-title" className="mt-4 text-lg font-bold tracking-tight text-zinc-950">Log out?</h2>
