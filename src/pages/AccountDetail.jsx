    ...
    <div className="mx-auto max-w-6xl px-5 py-6 sm:px-8 sm:py-8">
      ...
      <div className="mt-6 grid items-start gap-8 lg:grid-cols-[1.05fr_0.95fr] xl:gap-10">
        ...
        <section className="lg:sticky lg:top-20">
          ...
          {seller && <button type="button" onClick={() => navigate(`/seller/${seller.user_id}`)} className="mt-5 flex w-full items-center gap-3 rounded-2xl border border-zinc-200 bg-white p-4 text-left shadow-sm transition hover:border-zinc-400 hover:shadow-md">{seller.avatar_url ? <img src={seller.avatar_url} alt={`${seller.display_name} profile`} className="h-12 w-12 shrink-0 rounded-xl bg-zinc-100 object-cover" /> : <span className="grid h-12 w-12 shrink-0 place-items-center rounded-xl bg-zinc-950 text-base font-black text-white">{seller.display_name.charAt(0).toUpperCase()}</span>}<p className="min-w-0 flex-1 truncate text-base font-black">{seller.display_name}</p><p className="shrink-0 text-sm font-black tabular-nums text-zinc-700">{Number(seller.total_sales || 0).toLocaleString('en-IN')} orders</p><ArrowRight className="h-4 w-4 text-zinc-400" /></button>}
