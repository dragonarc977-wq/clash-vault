...
function ListingCard({ account, game, seller }) {
  ...
  return <article onClick={() => navigate(`/account/${account.id}`)} className="shop-listing-card group relative flex h-full cursor-pointer flex-col overflow-hidden rounded-2xl border border-zinc-200/90 bg-white transition duration-200 hover:-translate-y-1">
    ...
    <div className="shop-listing-body flex flex-1 flex-col p-3">
      <h2 className="line-clamp-2 min-h-9 text-sm font-semibold leading-[18px] tracking-[-0.015em]"><Link to={`/account/${account.id}`} onClick={(event) => event.stopPropagation()} className="rounded focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-zinc-950">{title}</Link></h2>
      {accountStats.length > 0 && <div className="mt-2.5 grid grid-cols-4 gap-1">{accountStats.slice(0, 4).map((stat) => <span key={stat.label} title={`${stat.label}: ${stat.value}`} className="shop-listing-stat min-w-0 rounded-lg border border-zinc-200 px-1 py-1.5 text-center text-zinc-600"><span className="block truncate text-[7px] font-medium uppercase tracking-[0.03em] text-zinc-500">{stat.label}</span><span className="mt-0.5 block truncate text-xs font-semibold text-zinc-900">{stat.value}</span></span>)}</div>}
